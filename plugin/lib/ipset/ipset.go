...
func (s *IPSet) Create() error {
	...
	out, err := exec.Command(ipsetPath, "create", s.Name, s.HashType, "family", s.HashFamily, "hashsize", strconv.Itoa(s.HashSize),
		"maxelem", strconv.Itoa(s.MaxElem), "timeout", strconv.Itoa(s.Timeout), "-exist").CombinedOutput()
	if err != nil {
		return fmt.Errorf("error creating ipset %s with type %s: %v (%s)", s.Name, s.HashType, err, out)
	}
	out, err = exec.Command(ipsetPath, "flush", s.Name).CombinedOutput()
	if err != nil {
		return fmt.Errorf("error flushing ipset %s: %v (%s)", s.Name, err, out)
	}
	return nil
}
